 ...
 * Verifies header presence (X-CSRF-Token or X-Requested-With) on state-changing requests (POST, PUT, DELETE, PATCH).
 */
export function csrfProtection(req: Request, _res: Response, next: NextFunction) {
  ...
  if (!csrfHeader && process.env.NODE_ENV === "production" && !req.path.includes("/auth/")) {
